...
@implementation Label_Text_view
...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        // 设置 dispatch source
        dispatch_source_set_timer(_timer, dispatch_walltime(NULL, 0), 0.016 * NSEC_PER_SEC, 0 * NSEC_PER_SEC);
...
{
    CGSize size = self.bounds.size;
    CGFloat width = [Label_Text_view widthWithString:_movingLabel.text font:_movingLabel.font];
    //    NSLog(@"widht = %f", width);
